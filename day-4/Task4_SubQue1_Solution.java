 ...
 */
public class Task4_SubQue1_Solution {

    public static void main(String[] args) {
        try {
            List<String> data = parseDataFile("src/data.txt");
            List<Room> rooms = parseData(data);
            List<Integer> validSectorIds = new ArrayList<Integer>();
            rooms.stream().filter(Task4_SubQue1_Solution::isValidRoom).forEach(Task4_SubQue1_Solution::rotateCharacters);
            rooms.stream().filter(Task4_SubQue1_Solution::isValidRoom).forEach((room->validSectorIds.add(room.sectorId)));
            int sum=0;
            for(int i=0; i<validSectorIds.size(); i++){
                sum += validSectorIds.get(i);
            }
